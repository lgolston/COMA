...
#ix = which( LGR_time >= datetime.strptime("2019-09-26 10:57:00","%Y-%m-%d %H:%M:%S"), 
#            LGR_time <= datetime.strptime("2019-09-26 13:50:00","%Y-%m-%d %H:%M:%S"))
...
for ii in range(12):
    ...
    mean_CO2 = np.mean(LGR_CO2[ix])
    values_CO = np.hstack((values_CO,LGR_CO[ix]-mean_CO))
