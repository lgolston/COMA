...
elif case == 1: # room air (last hr)
    filename_COMA = '../Data/2022-04-06/n2o-co_2022-04-07_f0000.txt'
elif case == 2: # scuba air
    filename_COMA = '../Data/2022-04-12/n2o-co_2022-04-12_f0000.txt'
elif case == 3: # chamber
    filename_COMA = '../Data/2022-05-19/n2o-co_2022-05-19_f0000.txt'
elif case == 4: # chamber
...
# %% plot CO and N2O
ix = np.ravel(np.where(LGR["      MIU_VALVE"]==7))
...
ax[0].plot(LGR_time[ix]+timedelta(hours=td),LGR["      [CO]d_ppm"][ix]*1000,'.',markersize=2)
ax[0].set_ylabel('CO, ppbv')
ax[0].set_ylim(225,242)

ax0_twin = ax[0].twinx()
ax0_twin.plot(CHAMBER_TIME,CHAMBER['PRESSURE'],'k',label='PRESSURE',alpha=0.5)
ax0_twin.set_ylabel('Chamber altitude, ft')

ax[1].plot(LGR_time[ix]+timedelta(hours=td),LGR["     [N2O]d_ppm"][ix]*1000,'.',markersize=2)
